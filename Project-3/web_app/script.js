...
async function getUsers() {
	let allFucntionCalls = await getAllFunctionCalls(contractAddress, "add_IOU");
	var setOfUsers = new Set();
	allFucntionCalls.forEach(element => {
		setOfUsers.add(element.from);
		setOfUsers.add(element.args[0]);
	})
...
async function getTotalOwed(user) {
	var total = await BlockchainSplitwise.get_total_owed(user);
	return total;
}
...
async function getLastActive(user) {
	let allFucntionCalls = await getAllFunctionCalls(contractAddress, "add_IOU");
	for (let element of allFucntionCalls) {
		if (element.from == user || element.args[0] == user) {
			return element.t;
		}
	}
...
provider.listAccounts().then((response)=> {
	defaultAccount = response[0].toLowerCase();
...
async function sanityCheck() {
	...
	var timeLastActive = await getLastActive(accounts[0].toLowerCase());
